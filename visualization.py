...
def run_visualization():
    ...
    def update(frame):
        ...
        # Debugging output for current position
        #print(f"Current position: {env.current_position}")

        # Ensure current_position is a sequence
        position = env.current_position.tolist() if isinstance(env.current_position, np.ndarray) else env.current_position

        #print(f"Position type: {type(position)}, Position: {position}")

        if not isinstance(position, (list, np.ndarray)) or len(position) != 2:
            raise RuntimeError(f"Invalid current position: {env.current_position}")

        line.set_data([p[0] for p in path], [p[1] for p in path])
        agent_marker.set_data([position[0]], [position[1]])

        wind_speed, wind_direction = env.get_wind_at_position(env.current_position)
        print(f"Step: {steps}, Position: {position}, Action: {action}, Reward: {rewards}, Done: {dones}, Info: {info}, Wind Speed: {wind_speed}, Wind Direction: {wind_direction}")
    ...
    anim = FuncAnimation(fig, update, frames=np.arange(500), repeat=True, interval=0.01)
    plt.show()
